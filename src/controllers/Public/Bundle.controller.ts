...
export default class ApiController {
    static async getDataBundlesAsAdmin(
        req: Request,
        res: Response,
        next: NextFunction,
    ) {
        const { productId } = req.query as Record<string, string>;
        const bundles = productId
            ? await BundleService.viewBundlesByProductId(productId)
            : await BundleService.viewAllBundles();
        res.status(200).json({
            status: "success",
    ...
    static async getSingleDataBundle(
        req: Request,
        res: Response,
        next: NextFunction,
    ) {
    ...
    static async getDataBundlesAsUser(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
        const { networkProvider } = req.query as Record<string, string>;
        const product = await ProductService.viewSingleProductByNameAndCategory(
            networkProvider,
            "DATA",
        );
        if (!product) {
            throw new BadRequestError("Invalid network provider");
        }

        const bundles = await BundleService.viewBundlesByProductId(product.id);
        const data = bundles.map((bundle) => {
            const { id, bundleCode, bundleName, bundleAmount, validity } =
                bundle;
            return { id, bundleCode, bundleName, bundleAmount, validity };
        });
    ...
    static async addNewBundle(
        ...
        if (!productId || !bundleCode || !bundleName || !amount || !vendorIds) {
            throw new BadRequestError(
                "ProductId, BundleCode, BundleName, Amount, Bonus and Commission are required",
            );
        }
        ...
        const bundle = await BundleService.addBundle({
            productId,
            bundleCode,
            bundleName,
            bundleAmount: amount,
