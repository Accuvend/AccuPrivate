...
static async generateAccessToken(
  ): Promise<ZohoIntegrationSettings | void> {
    try {
      ...
      const apiresponse = await fetch(`https://accounts.zoho.com/oauth/v2/token?code=${zohoSettings?.authorizationcode}&grant_type=authorization_code&client_id=${zohoSettings?.clientid}&client_secret=${zohoSettings?.clientsecret}&redirect_uri=${zohoSettings?.redirecturl}`, {
        method: "POST", 
        mode: "no-cors", 
        headers: {
      ...
      ${err.stack}` || `Error Generating Access Token 
      ${err.stack}`);
      throw err;
    }
  }
  ...
  static async refreshAccessToken(
    ...
    try {
      const zohoSettings: ZohoIntegrationSettings | null = await ZohoIntegrationSettings.findOne();
      if(zohoSettings == null) throw Error('Empty Zoho Setting');
      const apiresponse = await fetch(`https://accounts.zoho.com/oauth/v2/token?refresh_token=${zohoSettings?.refreshtoken}&grant_type=refresh_token&client_id=${zohoSettings?.clientid}&client_secret=${zohoSettings?.clientsecret}&redirect_uri=${zohoSettings?.redirecturl}`, {
        method: "POST", 
        mode: "no-cors", 
        headers: {
          "Content-Type": "application/json",
 ...
 */
  ...
  static async zohoAPICall(url: string , zohoSettings:ZohoIntegrationSettings, method: string | 'GET'   = 'GET' , body : Object | undefined = undefined ){
    try {
      let url_body: Object
      if(body){
        url_body = {
 ...
 */
  ...
  static async zohoEndpoint(
    url: string , method: string | 'GET'   = 'GET' , body : Object | undefined = undefined
  ){
    try {
      const zohoSettings: ZohoIntegrationSettings | null = await ZohoIntegrationSettings.findOne();
      if(zohoSettings == null) throw Error('Empty Zoho Setting');

      if(zohoSettings.accesstoken === '' || zohoSettings.refreshtoken === '' || zohoSettings.accesstoken === ' ' || zohoSettings.refreshtoken === ' '){ 
        await this.generateAccessToken()
      }

      const apidata = await this.zohoAPICall(url, zohoSettings, method , body)
            
      if(apidata?.error){
        throw Error(apidata?.error)
      }
      if(apidata?.errorCode === "INVALID_OAUTH") throw new Error('Expired Authetication token')
      ...
      try{
        ...
        if(error?.message === 'Expired Authetication token'){
          const updatedSetting: ZohoIntegrationSettings | void = await this.refreshAccessToken()


          if(updatedSetting == null) throw Error('Empty Zoho Setting');

          const apidata = await this.zohoAPICall(url, updatedSetting, method , body)
          if(apidata?.error){
            throw Error(apidata?.error)
          }
