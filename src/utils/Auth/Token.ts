...
type GeneratedCode<T extends AuthToken> = T extends "su_activation"
    ? `${IUUID}:${IUUID}:${IUUID}`
    : `${number}`;
...
class AuthUtil {
    static async generateToken(info: GenerateTokenData) {
        ...
        const tokenData: Omit<DecodedTokenData, "token"> = {
            user: {
                profile,
                entity: { ...entity.dataValues, role: role.name },
            },
            misc: { ...misc, tokenType: type },
        };
        const tokenKey = `${type}_token:${entity.id}`;
        const token = jwt.sign(tokenData, JWT_SECRET, {
            expiresIn: info.expiry,
        });
    ...
    static async generateCode<T extends AuthToken>({
        type,
        entity,
        expiry,
    }: Pick<GenerateTokenData<T>, "entity" | "type" | "expiry">): Promise<
        GeneratedCode<T>
    > {
